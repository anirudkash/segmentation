...
train_imgs_direc = 'TrainImgsOneWithBuilds'
train_imgs_labs_direc = 'MaskPatchesOneWithBuilds'
...
#model.load_weights('/home/anirud/Desktop/SemanticSeg/gq-weights/terrain-weights/weights_eight.weights.h5')

# Loads in the road weights
#model.load_weights('/home/anirud/Desktop/SemanticSeg/gq-weights/road-weights/revamped_two.weights.h5')
...
#model.load_weights('road_new.weights.h5')
#model.load_weights('/home/anirud/Desktop/SemanticSeg/good_weights_two.weights.h5') # used this last
...
for img in gq_imgs:
    ...
    color_masks = np.zeros((pixel_classes.shape[0], pixel_classes.shape[1], 3), dtype=np.uint8)
    
    for class_index in range(len(class_colors)):
        mask = pixel_classes == class_index
        color_masks[mask] = class_colors[class_index]

    cv2.imwrite(f'gq-segmented-road-classes/gq-{counter_str}.tiff', pixel_classes)
    cv2.imwrite(f'gq-segmented-road/gq-{counter_str}.tiff', color_masks)
